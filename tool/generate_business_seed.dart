...
void main() {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(biz['name']?.toString()));
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(desc));
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(biz['latitude']);
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(biz['longitude']);
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(biz['category']?.toString() ?? 'dining'));
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(biz['image_url']?.toString()));
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(biz['phone']?.toString()));
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(biz['email']?.toString()));
    buffer.writeln(',');
    buffer.write('  ');
    buffer.write(_escapeSqlString(biz['website']?.toString()));
    buffer.writeln(',');
    buffer.write('  ');
